...
DOCKER_COMPOSE_DEV := docker compose -f docker/compose.yml -f docker/dev/compose.yml -f compose.infra.yml -f compose.monitoring.yml
DOCKER_COMPOSE_TEST := docker compose -f docker/compose.yml -f docker/test/compose.yml -f compose.infra.yml -f compose.monitoring.yml
...
ifeq ($(PRIMARY_GOAL),swagger)
swagger: ## Generate OpenAPI documentation (JSON)
	$(DOCKER_COMPOSE_DEV) run --rm app ./vendor/bin/openapi src/Transport/Http/ApiDoc src/Transport/Http/ApiDoc/Schema -o public/openapi.json --format json
.PHONY: swagger
endif

ifeq ($(PRIMARY_GOAL),env-init)
env-init: ## Initialize .env files from .env.example
	@for dir in . docker docker/dev docker/test docker/prod; do \
		if [ -f "$$dir/.env" ]; then \
			echo "[SKIP] $$dir/.env already exists"; \
		elif [ -f "$$dir/.env.example" ]; then \
			cp "$$dir/.env.example" "$$dir/.env"; \
			echo "[OK]   $$dir/.env created"; \
		else \
			echo "[MISS] $$dir/.env.example not found"; \
		fi; \
	done
.PHONY: env-init
endif
